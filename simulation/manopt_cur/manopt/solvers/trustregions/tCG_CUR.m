...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 	NB Feb 05, 2016: 
% 		Slightly modified by Ju Sun to more directly exploit negative curvature 
%		if there is when the gradient is small 
...
if ~options.useRand % and therefore, eta == 0
	%% this if part modified by Ju Sun 
	grad_sq = inner(x, grad, grad); 
	norm_grad = sqrt(grad_sq); 
	if norm_grad <= options.tolgradnorm * 10 
		options.solver = @steepestdescent;   % by default, it uses the TRM solver, and recursion occurs 
		options.tolcost = -options.tolgradnorm * 100;   % early stop when a negative curvature direction is identifified 
		options.verbosity = 0; 
		[v, lambda] = hessianextreme(problem, x, 'min', [], options, storedb, key); 
		
		if real(lambda) <= -eps    % near saddle or maximizer 
			gh_inner = inner(x, grad, v); 
			if abs(gh_inner) >= eps 
				r = sign(gh_inner) * v; 
			else 
				r = v; 
